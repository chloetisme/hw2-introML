...
def run_cv(K, maxorder, x, t, independent_testx, independent_testt,
    ...
    for p in range(maxorder + 1):
        ...
        for fold in range(K):
            ...
            X_trans = numpy.transpose(trainX)
                                
            w = numpy.dot((numpy.dot(numpy.linalg.inv(numpy.dot(X_trans,trainX)),X_trans)),traint)
...
# -----------------------------------------------------------------------------
# Basically a copy of "run_cv" without independent tests
# -----------------------------------------------------------------------------

def run_cv_solution(K, maxorder, x, t, randomize_data=True, title='CV', figure_path+none):
    """
    Implements the core of the cross-validation demonstration
    from FCML Ch 1, pp.31-32.
    :param K: Number of folds
    ...
    :param randomize_data: Boolean (default False) whether to randomize the order of the data
    :param title: Title for plots of results
    :param figure_path: path where figure will be saved
    :return:
    """
    ...
    #removed for 5 fold and LOOCV questions
    # Design matrix for independent test data
    #independent_testX = numpy.zeros((independent_testx.shape[0], maxorder + 1))
    ...
    return best_poly, min_mean_log_cv_loss, w
...
def exercise_3_5fold(data_path, figure_path, seed):
    ...
    k5_best_model_order, k5_best_CVtest_log_MSE_loss, k5_best_model_w = \
        run_cv_solution( K, 7, x, t, randomize_data=True, title=f'{K}-fold CV' )
...
def exercise_3_LOOCV(data_path, figure_path_LOOCV, seed):
    ...
    loocv_best_model_order = None
    # Calculate the log MSE CV test loss for the best polynomial model
    #       order on the synthetic data
    loocv_best_CVtest_log_MSE_loss = None
    # Report the best-fit parameters w for teh best model order
    loocv_best_model_w =None
    ...
    loocv_best_model_order, loocv_best_CVtest_log_MSE_loss, loocv_best_model_w = \
        run_cv_solution( K, 7, x, t, randomize_data=True, title=f'{K}-fold CV' )
